...
public class Student_BST {
    ...
    private Node insertRec(Node p, Student newStudent) {
         if (p == null) {
            p = new Node(newStudent);
        } else if (newStudent.getId() < p.info.getId()) {
            p.left = insertRec(p.left, newStudent);
        } else if (newStudent.getId() > p.info.getId()) {
            p.right = insertRec(p.right, newStudent);
        }
        return p;
    }
